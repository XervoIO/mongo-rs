...
for db in \${DATABASES[@]}; do
  if [[ \$db == "local" ]]; then
     \$DUMP_TOOL --username \$MONGO_ROOT_NAME --password \$MONGO_ROOT_PASSWORD --host \$HOST --archive=\$BACKUP_DIR/\$db --gzip --authenticationDatabase admin --db \$db --excludeCollection oplog.rs &>> \$LOG_FILE
  else
     \$DUMP_TOOL --username \$MONGO_ROOT_NAME --password \$MONGO_ROOT_PASSWORD --host \$HOST --archive=\$BACKUP_DIR/\$db --gzip --authenticationDatabase admin --db \$db &>> \$LOG_FILE
  fi
done

# Tar up the backups.  No need to compress as the files will already be compressed by the dump tool
tar -cf \$TAR \$BACKUP_DIR &>> \$LOG_FILE
